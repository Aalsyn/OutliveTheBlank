...
{
	pSwapChain->SetFullscreenState(true, nullptr); // passer en mode fenêtré
	...
	const HWND hWnd)
{
	...
	sd.SampleDesc.Count = AASampleCount;
...
void CDispositifD3D11::InitDepthBuffer()
{
	...
	depthTextureDesc.SampleDesc.Count = AASampleCount;
	...
	DXEssayer(pD3DDevice->CreateDepthStencilView(pDepthTexture, 0, &pDepthStencilView), DXE_ERREURCREATIONDEPTHSTENCILTARGET);
}
