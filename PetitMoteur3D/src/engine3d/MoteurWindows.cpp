...
namespace PM3D
{
	...
	const Horloge& CMoteurWindows::GetHorloge() {
		return m_Horloge;
	}
	...
	LRESULT CALLBACK CMoteurWindows::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_COMMAND:
			...
			case IDM_EXIT:
				DestroyWindow(hWnd);
				break;
			case IDM_CLEARVIEWPORT:
			{
				CMoteurWindows::GetInstance().ListeScene.clear();
				
				CTerrainHandler::get().release();
				break;
			}
			case IDM_IMAGELOAD:
			{

				// ask for hightmap image path
				std::string hmPath;
				if (!FileHandler::get().retrieveImagePathFE(hWnd, hmPath)) {
					break;
				}
				CTerrainHandler& tg = CTerrainHandler::get();
				tg.release();

				// generate 3d data from hight map
				tg.generateTerrain(hmPath);

				// add mesh to the scene
				// Lui assigner une texture
				CMoteurWindows::GetInstance().ListeScene.emplace_back(
					std::make_unique<CTerrain>(
						tg.pVertices,
						tg.pIndices,
						tg.verticesSize,
						tg.indicesSize,
						...
						tg.echelleXZ,
						tg.echelleY,
						CMoteurWindows::GetInstance().pDispositif)
				);
			...
			case IDM_TXTEXPORT:
			{
				...
				if (!FileHandler::get().retrieveMeshSavingPathFE(hWnd, meshFilePath)) {
					break;
				}
				CTerrainHandler& tg = CTerrainHandler::get();
			...
			case IDM_TXTIMPORT:
			{
				// ask for a model stored in a .txt, clear existing model and then load .txt file
				std::string meshLoadingFilePath;
				if (!FileHandler::get().retrieveMeshLoadingPathFE(hWnd, meshLoadingFilePath)) {
					break;
				}
				...
				// if file was empty or could not be read cancel operation
				if (!FileHandler::loadMeshFromTXTFile(meshLoadingFilePath)){
					break;
				}
			...
			case IDM_SWITCHCAM:
				CMoteurWindows::GetInstance().GetCamera()->switchCameraMode();
				break;
			default:
